...
ImageDataPtr Projector::PerformProjection( ImageDataPtr p_volume ) const
{
    ...
    {
        projectionsOriginInWorld[projectionIndex].x = m_tomoGeometry->projectionsRoisBottomLeftPositionsPtr()->at( projectionIndex ).x;
        projectionsOriginInWorld[projectionIndex].y = m_tomoGeometry->projectionsRoisBottomLeftPositionsPtr()->at( projectionIndex ).y;
    ...
    for( auto projectionIndex{ 0 }; projectionIndex < projectionDimensions.z; projectionIndex++ )
    {
        ...
        sourcePositionsInWorld[projectionIndex].y = m_tomoGeometry->sourcesYPositionsPtr()->at( projectionIndex );
    ...
    auto outputImage = ImageDataPtr::New();
...
ImageDataPtr Projector::PerformBackProjection( ImageDataPtr p_projections ) const
{
